...
if st.session_state.page == "home":
    ...
    st.write("Unsere App nutzt moderne Machine-Learning-Algorithmen. Sobald du ein Bild hochlädst, analysiert die KI das Foto und ordnet es automatisch einer Kategorie (z. B. Hoodie, T-Shirt, Jacke) zu. So sparen wir Zeit bei der Verwaltung!")
    ...
    if uploaded_file is not None:
        ...
        if "current_prediction" in st.session_state:
            ...
            with st.form("upload_form"):
                ...
                description = st.text_area("Beschreibung", placeholder="z.B. Reißverschluss leicht beschädigt, Name im Etikett...")
                special_features = st.text_input("Besonderheiten", placeholder="z.B. Schlüssel in der Tasche")
                contact = st.text_input("Kontakt für Rückfragen (optional)", placeholder="z.B. Hausmeister / Lehrkraft")
...
# ==========================================
    ...
    if not item:
        ...
        with col_info:
            ...
            st.markdown(f"**KI-Vorhersage:** {item.get('ai_prediction')} ({float(item.get('ai_confidence', 0))*100:.1f}%)")
        ...
        with st.form("claim_form"):
            ...
            claim_reason = st.text_area("Warum gehört das Kleidungsstück dir? (z.B. Name im Schild, Verlustdatum)")
            submit_claim = st.form_submit_button("✉️ ANFRAGE ABSENDEN")

            if submit_claim:
                if claim_name and claim_email and claim_reason:
                    st.success("Vielen Dank! Deine Anfrage wurde aufgenommen. Bitte wende dich zur Abholung an das Sekretariat.")
                else:
...
# ==========================================
    ...
    # Passwortprüfung über Secrets
    secret_pass = st.secrets.get("admin", {}).get("password", None)
    if not secret_pass:
        st.warning("⚠️ Kein Admin-Passwort in `.streamlit/secrets.toml` konfiguriert! Standard-Modus aktiviert.")
        secret_pass = "admin"
        ...
        with tabs[0]:
            ...
            for item in items:
                with st.expander(f"[{item['status']}] {item['id']} - {item['category']} ({item['color']})"):
                    c1, c2 = st.columns(2)
                    with c1:
                        new_status = st.selectbox("Status ändern", ["Verfügbar", "Reserviert", "Abgeholt", "Nicht zugeordnet", "Archiviert"], index=["Verfügbar", "Reserviert", "Abgeholt", "Nicht zugeordnet", "Archiviert"].index(item.get("status", "Verfügbar")), key=f"stat_{item['id']}")
